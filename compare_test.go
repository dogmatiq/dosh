...
var _ = Describe("type Amount (comparison functions)", func() {
	Describe("func IsZero()", func() {
		DescribeTable(
			...
			func(dec string, expect bool) {
				Expect(FromString("XYZ", dec).IsZero()).To(Equal(expect))
			},
			Entry("zero", "0", true),
	...
	Describe("func IsPositive()", func() {
		DescribeTable(
			...
			func(dec string, expect bool) {
				Expect(FromString("XYZ", dec).IsPositive()).To(Equal(expect))
			},
			Entry("zero", "0", false),
	...
	Describe("func IsNegative()", func() {
		DescribeTable(
			...
			func(dec string, expect bool) {
				Expect(FromString("XYZ", dec).IsNegative()).To(Equal(expect))
			},
			Entry("zero", "0", false),
			Entry("positive", "1.23", false),
